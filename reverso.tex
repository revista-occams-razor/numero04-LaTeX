...
%\rput(4.0,-2.1){\resizebox{19cm}{!}{{\epsfbox{images/reverso/header.eps}}}}
...
%\msubtitle{12cm}{Descubre como trabajan las herramientas de los
%hackers}
...
permitirá introducir los nuevos conceptos de una forma más progresiva.
...
encontramos los de siempre, ficheros estándar para utilizar funciones
...
llamado \verb!icmp_hello_log!. Desde otro terminal, hacemos un \verb!ping! a
...
{\small
\begin{verbatim}
occams@razor $ sudo ./icmp_hello_log
...
sencillo \verb!ping! vamos a hacer un pequeño inciso, 
...
A partir de en nuestro ejemplo anterior, vamos a realizar unas pequeñas
...
Como podéis observar, cuando creamos nuestro nuevo socket RAW, ahora
...
Ahora, pulsad CTRL+C en la parte cliente y ved que obtenéis en el
...
Después de este ``no tan corto paréntesis'', volvamos a nuestro
...
el primer ejemplo que introdujimos (\verb!icmp_hello_log!). También
reutilizaremos nuestra función \verb!crea_socket!, pero para poder transmitir
nuestro paquete ICMP necesitaremos algo más de código. 
...
de nuestro sistema rellene los campos de la cabecera IP. Más adelante veremos como
rellenarlos nosotros mismos, pero para implementar nuestro \verb!ping!, es
mucho más conveniente dejar al sistema hacerlo.
...
\verb!checksum! de la cabecera igual a 0.
...
un terminar. Esto es lo que obtendríamos:
...
interesante \verb!IPPRPTO_TCP!, como parámetro a nuestra función de
creación del socket.
...
para generar esos números de serie no eran muy buenos, y había formas
de llegar a predecir esos números de forma relativamente sencilla.
...
construir la mayoría de las herramientas del sistema relacionadas con
