...
es la única soportada por todos los drivers.
...
estaba en la función ``gspca\_init\_transfert''. 
...
Esta nueva variable la utilizamos en la función \verb!gspca\_init\_transfert! para
modificar el valor de la variable \verb!nbalt!, de la siguiente forma:
...
estándar como camorama o cheese.
...
supersimplificada, nuestro módulo tendrá 3 funciones. Aquí tenéis los prototipos.
...
la llamada al sistema ioctl. Para la configuración del canal,
...
características del dispositivo, la información sobre sus canaleso. En
...
campo channel de la estructura de datos \verb!video_channel! (variable
...
read y otra es utilizando mapeado de memoria con la llamada al sistema
mmap.
...
ioctl. En la estructura \verb!video_mbuf! (uno de los campos de nuestra
...
``escupir'' imágenes... pero como las obtenemos?... enseguida lo
veremos.
...
nuestro dispositivo. A continuación tenemos que hacer un pequeño
...
las imágenes, implica manipular los pixels de ésta... y eso es lo que
hacen esos dos bucles tras la captura. 
...
La función gfx\_pinta utiliza OpenGL para mostrar el vídeo en tiempo
...
Así, la técnica consiste en lo siguiente. Primero generamos e
...
utiliza en la función main y se llama init\_texture. La segunda se
utiliza cuando capturamos nuestras imágenes y se llama,
change\_texture. Y finalmente, la última la acabamos de ver en la
sección anterior y se llama square\_texture.

Veamos cada una de ellas por separado.

\sectiontext{white}{black}{INICIALIZANDO LAS TEXTURAS}

La función init\_texture se encarga de la inicialización de las
...
La función init\_texture es la encargada de hacer esto. La hemos
...
máximo de 4 de ellas y almacena sus identificadores en una variable
global del módulo llamada texid. 

Lo primero que hacemos es decirle a OpenGL que necesitamos nuestras
cuatro texturas y que nos almacene los identificadores en la matriz
texid. A continuación, tenemos que inicializar cada una de ellas. 
...
glTexParameteri... está claro no? :).


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\ebOpage{introcolor}{0.25}{MÚ RÁPIDO}

Los dos parámetros que hemos dejado le dicen a OpenGL que filtro
...
simplemente cambiando este parámetro de GL\_NEAREST a GL\_LINEAR.

Finalmente, la función glTexImage2D es la que nos permite definir el
...
textura. Esto lo hacemos con la función change\_texture que podéis ver
a continuación
...
Esta es sencilla no?. La función glBindTexture le dice a OpenGL que
...
glTexSubImage2D, que es exactamente igual a la función glTexImage2D
...
utilizar, con la habitual llamada a glBindTexture. A continuación,
...
La función glTexCoord2f, nos permite seleccionar un punto dentro de la
...
sustituir el valor 1.0 en glTexCoord2f, por el valor apropiado, que
...
que comentar una última función, gfx\_init. Esta función se encarga de
...
glClear (la primera llamada en gfx\_pinta).
