...
suerte el programa que lo proporciona y su versión).

Vamos, una herramienta muy útil.

\sectiontext{white}{black}{RADIOGRAFÍA DE UN ATAQUE}
...
nivel que el protocolo IP. ICMP es el acrónimo de Internet Control
Message Protocol, es decir, protocolo de mensajes de control de
Internet. Y eso es precisamente lo que hace, enviar y recibir mensajes de
control. Los que queráis ver la lista completa de todos esos
mensajes, podéis consultarlos en el RFC 792, el documento que describe
este protocolo.
...
generarse localmente (como en el caso de la red inalcanzable, este
...
Toman los datos de una capa superior que en última estancia es el
...
la comunicación fiable entre dos máquinas remotas (eso es lo que hacen
...
\item \verb!SYN!. Este es el tipo de paquete utilizado para establecer
  conexiones. Nos vamos a aburrir de él :)
...
\begin{entradilla}
{\em Los {\color{introcolor}paquetes SYN, ACK y RST} son los que interesan}
\end{entradilla}
...
\item Regla 3. Si enviamos un paquete ``ilegal'' a una máquina que no existe,
recibiremos un paquete \verb!RST!
\end{itemize}
...
detalle sobre como llamar a los telefonillos, evitar porteros y hacer
todo esto sin que se note :)

El primer paso, antes de llevar a cabo ningún tipo de acción más
complicada, es determinar si la máquina a analizar existe y esta
encendida. Este proceso se suele extender a analizar rangos de
direcciones IPs, ya que quizás la máquina que interesa al intruso este
...
Así, lo primero que tenemos que averiguar es como saber si una determinada
...
La primera forma es el denominado ``ping scan'', que básicamente hace lo mismo
que la herramienta ping. Y como funciona ping? os preguntaréis. Bueno,
...
componer el paquete específicamente para cada prueba. De la misma
...
opción, además envía un paquete TCP al puerto 80, de forma que un
usuario no privilegiado pueda utilizar la aplicación. Enseguida
veremos como.
...
algunos comentarios. El primero es relativo al uso de el flag
...
determinar si la máquina esta funcionando.

El segundo comentario es relativo a porqué enviar un paquete TCP al
...
esta levantado, pero nmap nos proporciona una serie de opciones
...
máquinas. La página del manual de nmap proporciona un montón de
...
sobre que ``ping'' hace cada una de ellas.
...
En este caso, nmap mantiene una base de datos de direcciones MAC
associadas a fabricantes, siendo capaz, en algunos casos, de
...
La principal función de nmap es informar de que puertos en la máquina
víctima están abiertos, pero, como ya adelantamos, también es capaz de
proporcionar un montón de información adicional muy util. Todo esto lo
...
máquina destino. Nosotros vamos a utilizar el puerto https de nuestro
router ADSL utilizado para administración, pero si disponéis de otra
...
básicamente tiene ambos flags, \verb!SYN! y \verb!ACK! activados en
sus cabeceras. 
...
adicional que sale en cada una de las líneas. Bien, todos esos datos
...
utilizados para evitar lo que se conoce como ``spoofing''.

\sectiontext{white}{black}{SPOOFING EXPLICADO} 

Ya que ha salido vamos a explicar mú rápido en que consiste el
...
dirección falsa y por lo tanto el sistema del spoofer no va a poder
verlos.
...
tiene que enviar un nuevo paquete \verb!ACK! para compretar la conexión, como
...
remota nos iba a enviar y por tanto poder establecer la conexión aún
cuando no se capturara el paquete \verb!SYN ACK!.

Nmap nos da algunas pistas sobre esto, y aunque hablaremos sobre el
tema más adelante, podéis probar lo siguiente:

Si, la opción \verb!-O! habilita la detección del sistema operativo, que
...
estuviera abierto... pero que sucede cuando el puerto está
...
de escaneo es obvio. Porque completar el proceso de conexión con el
...
clara. Sino estáis en una red local podéis prescindir de este flag.

Lo que nuestro sniffer capturar en este caso es lo esperado. El
...
lo detectan. Muchos servidor graban en su log información sobre las
...
El proceso consiste en enviar un paquete UDP vacio al puerto que se
...
independientemente por la red y tampoco existen paquetes ACK que el
...
o el cortafuegos a tirado nuestro paquete.
...
averiguar si una puerto esta abierto o no de forma sigilosa.
...
segundo test. La Figura 12 muestra que obtendríamos si dirigimos nuestro escaneo a un
puerto cerrado.
...
13:31:58.833375 IP razor.www > eve.52797: R 0:0(0) ack 3074120381 win 0
...
embargo, como sucede con toda reglas, siempre hay excepciones.
...
puñado de ellos, probablemente ese los que componen ese puñado sean
...
{\textbf{\textsf{CORTAFUEGOS CON Y SIN ESTADO}}}

{\textsf{Como todos sabéis, un cortafuegos o firewall, permite controlar el
...
por paquetes \verb!SYN!, sin embargo, los paquetes \verb!ACK! no se
...
recibe un paquete \verb!ACK! cuyo número de serie no se corresponde con
...
documentación de nmap se refiere como ``maquina zombie''.
...
Un incremento de 1 en el ID significa que la máquina no ha enviado
...
una buena elección. Si, eso todavía no lo hemos dicho, pero la máquina
zombie debe ser una que no tenga apenas actividad de red, de forma que
durante el escaneo, no envié ningún paquete y la monitorización del IP
ID sea efectiva.
...
desactivada esta funcionalidad, porque no es muy práctica en la
actualidad... pero siempre hay algún despistado por ahí. Lo que
...
\sectiontext{white}{black}{PONIENDOLO DIFÍCIL} 
...
proporciona la opción. La Figura 13, muestra lo que obtenemos al
